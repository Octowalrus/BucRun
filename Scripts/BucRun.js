...
let startTime = 0; // Variable to store the time when the game starts (used for score calculation)
...
CANVAS.addEventListener("click", (e) => {
  ...
  if (currentScreen === "menu") {
    ...
    if (
      ...
      score =0; // reset score at the beginning of a run
      startTime = Date.now(); // Record the start time to calculate score based on survival time
    }
...
function update(dt) {
  ...
  // Calculate score based on how long the player has survived
  // Date.now() returns the current time in milliseconds
  // Subtract startTime to get elapsed time, then convert to seconds
